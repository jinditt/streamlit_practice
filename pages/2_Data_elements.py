...
st.header('2.data editor: st.data_editor()') # 편집 가능
# 데이터프레임과 데이터에디터: 수정 가능/ 위젯 사용 여부에 따라 차이
...
st.markdown('num_rows=True')# 행 추가 가능
st.data_editor(df3_part1, num_rows='dynamic')

st.markdown('disabled = [col,...]')
st.data_editor(df2, disabled=['command'])

st.header('3. table: st.table()')
# st.table(df3_part) # 정적인 형태의 테이블. 너무 길어
# 데이터프레임과 테이블의 큰 차이점: 테이블은 내용을 다 보여줌.
st.table(df3_part.iloc[:3])

out_table = st.table(df3_part1)
out_table.add_rows(df3_part2)

# 실행하는 법: streamlit run 2_Data_elements.py
